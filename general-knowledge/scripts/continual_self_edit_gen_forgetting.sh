...
INDEX=0
MODEL_NAME="models/iter2" # Qwen/Qwen2.5-7B, models/iter1, models/iter2
...
case "${INNER_MODE}" in
    single)
        # continual_self_edits-style: per-passage inner SFT + n_val held-out cols
...
OUTPUT_DIR="general-knowledge/results/continual_self_edit_gen_forgetting/${INNER_MODE}/run${INDEX}"
mkdir -p "${OUTPUT_DIR}"
...
echo "  split_newlines=${SPLIT_NEWLINES}"
echo "==================================="
...
INNER_RESULTS_DIR="general-knowledge/results/continual_self_edits/${INNER_MODE}/run${INDEX}"
PLOT_ARGS=(--results_dir "${OUTPUT_DIR}" --output "${OUTPUT_DIR}/forgetting_heatmap_se_gen.png")
if [[ "${INNER_MODE}" == "single" ]] \
    && [[ -d "${INNER_RESULTS_DIR}" ]] \
    && compgen -G "${INNER_RESULTS_DIR}/summary_*.json" > /dev/null; then
    PLOT_ARGS+=(--inner_results_dir "${INNER_RESULTS_DIR}")
    PLOT_ARGS+=(--combined_output "${OUTPUT_DIR}/forgetting_heatmaps_combined.png")
fi
python3 general-knowledge/src/continual/plot_self_edit_gen_forgetting.py "${PLOT_ARGS[@]}"
